...
# Wrapper for: email-nurse ops stuck-check
# Runs on a schedule to detect and recover from stuck messages.
...
log() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [ops:stuck] $1"
}
...
log "Starting stuck-check"

if "$INSTALL_DIR/venv/bin/email-nurse" ops stuck-check -v; then
    log "SUCCESS"
else
    EXIT_CODE=$?
