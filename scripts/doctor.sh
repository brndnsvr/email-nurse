...
# LaunchAgent names
INTERVAL_PLIST="com.email-nurse.plist"
WATCHER_PLIST="com.email-nurse-watcher.plist"
REPORT_PLIST="com.email-nurse-report.plist"
...
check_launchagents() {
    ...
    if [[ -f "$PLIST_DIR/$INTERVAL_PLIST" ]]; then
        local agent_state=$(get_launchctl_state "com.email-nurse")
    ...
    if [[ -f "$PLIST_DIR/$WATCHER_PLIST" ]]; then
        local agent_state=$(get_launchctl_state "com.email-nurse-watcher")
        if [[ -n "$agent_state" ]]; then
            if [[ "$agent_state" == "-" ]]; then
    ...
    if [[ -f "$PLIST_DIR/$REPORT_PLIST" ]]; then
        local report_state=$(get_launchctl_state "com.email-nurse-report")
