...
LAUNCH_AGENT_NAME="com.email-nurse.plist"
...
restart_agent() {
    ...
    if ssh "$TARGET_HOST" "launchctl stop com.email-nurse 2>/dev/null; launchctl start com.email-nurse"; then
        log_ok
        return 0
    else
        log_fail
...
restart_watcher() {
    ...
    local service="com.email-nurse-watcher"
...
if $NO_AGENT; then
    ...
    if ! $DRY_RUN; then
        ...
        ssh "$TARGET_HOST" "launchctl load ~/$LAUNCH_AGENT_DIR/$LAUNCH_AGENT_NAME && launchctl start com.email-nurse"
    fi
    log_ok
fi
...
if $DRY_RUN; then
    ...
    # Check LaunchAgent
    if ! $NO_AGENT; then
        AGENT_STATUS=$(ssh "$TARGET_HOST" "launchctl list | grep -q com.email-nurse && echo loaded || echo not_loaded")
