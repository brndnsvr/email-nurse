...
- **Work**: AppleScript fallback (Work uses "Inbox" not "INBOX")
...
4. **Re-enabled sysm** (22:00): Full sysm mode — discovered Work "Inbox" vs "INBOX" mismatch
5. **Switched to hybrid** (22:10): sysm for iCloud/Gmail, AppleScript fallback for Work
...
| Gmail | sysm | 16 | 2.0s | |
| Work | applescript | 150 | 34.6s | Fallback (mailbox name mismatch) |
...
1. **Work mailbox name mismatch**
   - Work account uses "Inbox" (not "INBOX")
...
- [x] Work falls back to AppleScript gracefully
- [x] Quick rules still work (monitoring, Reddit)
